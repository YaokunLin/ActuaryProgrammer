...
def insert_voip_provider_with_pk():
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            [settings.PEERLOGIC_VOIP_PROVIDER_ID],
        )
    try:
...
def insert_practice_with_pk():
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            [settings.PEERLOGIC_PRACTICE_ID],
        )
    try:
        practice = Practice.objects.get(pk=settings.PEERLOGIC_PRACTICE_ID)
...
def insert_practice_telecom_with_pk(practice):
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            [settings.PEERLOGIC_PRACTICE_ID, practice],
        )
    try:
        practice_telecom = PracticeTelecom.objects.get(pk=settings.PEERLOGIC_PRACTICE_TELECOM_ID)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            self.stdout.write(f"Getting practice with {settings.PEERLOGIC_PRACTICE_ID}")
            practice = Practice.objects.get(pk=settings.PEERLOGIC_PRACTICE_ID)
            self.stdout.write(self.style.SUCCESS(f"Got practice with {practice.pk}"))
        except Practice.DoesNotExist:
            self.stdout.write(f"Creating practice with {settings.PEERLOGIC_PRACTICE_ID}")
            insert_practice_with_pk()
            self.stdout.write(self.style.SUCCESS(f"Created practice with {settings.PEERLOGIC_PRACTICE_ID}"))

        try:
            self.stdout.write(f"Getting practice telecom with {settings.PEERLOGIC_PRACTICE_TELECOM_ID}")
            practice_telecom = PracticeTelecom.objects.get(pk=settings.PEERLOGIC_PRACTICE_TELECOM_ID)
            self.stdout.write(self.style.SUCCESS(f"Got practice telecom with {practice_telecom.pk}"))
        except PracticeTelecom.DoesNotExist:
            self.stdout.write(f"Creating practice telecom with {settings.PEERLOGIC_PRACTICE_TELECOM_ID}")
