...
def netsapiens_call_subscription_view(request):
    ...
    netsapiens_callid = request.data[0]["orig_callid"]  # Will see if this is correct
    if settings.NETSAPIENS_ETL_CALL_MODEL_SUBSCRIPTION_TYPE == "call":
        NetsapiensCdrsExtract.objects.create(netsapiens_callid=netsapiens_callid, netsapiens_cdrs_extract=request.data)
        log.info(f"Extract data for call id {netsapiens_callid} from Call subscription saved to netsapiens etl cdrs extract.")
    else:
        log.info(f"NETSAPIENS_ETL_CALL_MODEL_SUBSCRIPTION_TYPE not set to call. Not saving to database for call id {netsapiens_callid}")
...
def netsapiens_call_origid_subscription_view(request):
    ...
    if settings.NETSAPIENS_ETL_CALL_MODEL_SUBSCRIPTION_TYPE == "call_origid":
        NetsapiensCdrsExtract.objects.create(netsapiens_callid=netsapiens_callid, netsapiens_cdrs_extract=request.data)
        log.info(f"Extract data for call id {netsapiens_callid} from Call ORIG id subscription saved to netsapiens etl cdrs extract.")
    else:
        log.info(f"NETSAPIENS_ETL_CALL_MODEL_SUBSCRIPTION_TYPE not set to call_origid. Not saving to database for call id {netsapiens_callid}")
