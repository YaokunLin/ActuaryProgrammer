...
def requires_auth(func) -> Callable:
    ...
    def wrapper(self, *args, **kwargs) -> requests.Response:
        ...
        try:
            ...
            msg = f"Problem occurred with calling peerlogic api with args='{args}', kwargs='{kwargs}' %s"
...
def raise_for_bad_auth_and_other_statuses(response: requests.Response) -> requests.Response:
    ...
    try:
        ...
        try:
            # attempt to get information from the response object if possible
            log.info(f"Raised for status and found error: {response.json()}")
        except:
            pass  # ignore since we can't fix it and we bubble up anyway
        raise e
