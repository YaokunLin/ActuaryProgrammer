...
from devtools.api_clients.api_client import APIClient
...
class NetsapiensAPIClient(APIClient):
    def __init__(self, root_api_url: str) -> None:
        ...
        self._netsapiens_cdr2_url = root_api_url

    def get_auth_url(self) -> str:
        return self._netsapiens_auth_url

    def get_cdr2_url(self) -> str:
        return self._netsapiens_cdr2_url
    ...
    def get_cdr2(self, orig_callid: str, term_callid: str, session: requests.Session = None) -> Dict:
        try:
            ...
            params = {"object": "cdr2", "action": "read", "format": "json", "limit": 20, "orig_callid": orig_callid, "term_callid": term_callid}
            url = self.get_cdr2_url()
    ...
    def get_recording_urls(self, orig_callid: str, term_callid: str, session: requests.Session = None) -> HTTPResponse:
        try:
            response = None  # define for proper logging as needed
            if not session:
                session = self.get_session()
            ...
            log.info(f"Retrieved recording url response: '{response.text}'")
            return response
        except Exception as e:
            msg = f"Problem occurred getting recording urls from url='{url}', params='{params}'."
            if response is not None and response.text:
                msg = f"{msg}. Response text: '{response.text}'. Response code: '{response.status_code}'"
            raise Exception(msg) from e
